...
module.exports = class Router {
  ...
  constructor({history, errorComponent, routes}) {
    ...
    this.currentParams = {};
    this.currentResolveData = {};
    this.currentProps = {};

    this.onHistoryChange = this.onHistoryChange.bind(this);
    this.start = this.start.bind(this);
    this.registerRouterView = this.registerRouterView.bind(this);
    this.reload = this.reload.bind(this);
    this.go = this.go.bind(this);
    this.renderError = this.renderError.bind(this);
    this.broadcastStartEvent = this.broadcastStartEvent.bind(this);
    this.broadcastSuccessEvent = this.broadcastSuccessEvent.bind(this);
    this.broadcastErrorEvent = this.broadcastErrorEvent.bind(this);
  ...
  onHistoryChange({location, action}) {
    ...
    let isBackToParent = previousRoute && previousRoute.name ?
      previousRoute.name.indexOf(`${nextRoute.name}.`) === 0 && !isReloadNextHistoryChange :
      true;
    ...
    if (this.promise && !isReloadNextHistoryChange) {
      ...
      for (let index = 0; index < nextRouteChain.length; index += 1) {
        ...
        if (route.name === (this.views[index] && this.views[index].name)) {
    ...
    this.promise = this.broadcastStartEvent({
      ...
      .then(resolveData => {
        if (!isBackToParent && !nextRouteChain[changeViewIndex]) {
          // Route back to previous functional page and replace browser history
          console.log('resolveData onHistoryChange', resolveData);
          console.error('Router change error: receive no result when fetchResolveData.\r\nNote: When the user go to the other route before the promise was done, the old one will throw null.');
          const previousUri = previousRoute.generateUri(previousParams || {});
          ...
          return [action, nextRoute, nextParams, previousRoute, previousParams, this.currentProps];
        }
        ...
        if (isBackToParent && this.views[changeViewIndex]) {
          ...
          if (!previousRoute || previousRoute.dismissalDelay == null) {
            dismiss();
          } else {
            setTimeout(dismiss, previousRoute.dismissalDelay);
          }
        } else if (this.views[changeViewIndex]) {
      ...
      .catch(error => {
        ...
        if (error instanceof TypeError) {
          ...
          // When do history forward/backward quickly, sometimes it will cause the promise error on undefined result.
          // Skip this error until a reliable solution were found.
          return;
        }
  ...
  registerRouterView(routerView) {
    const currentRoute = this.currentRoute || this.getCurrentRoute();
    const routeChain = [...currentRoute.parents, currentRoute];
  ...
  reload() {
    ...
    this.promise = this.broadcastStartEvent({
      ...
      .then(resolveData => {
        ...
        return [historyActions.RELOAD, route, params, route, params, props];
      })
      .catch(error => {
        if (error instanceof errors.URLChangedError) {
          return;
   ...
   */
  broadcastStartEvent({action, previousRoute, previousParams, nextRoute, nextParams}) {
    return new Promise(resolve => {
      ...
      if (action !== historyActions.INITIAL) {
        fromState = {
          name: previousRoute && previousRoute.name,
