  ...
  fetchResolveData: (route, params, reusableResolveData, history) => {
    ...
    routeChain.forEach(route => {
      Object.keys(route.resolve).forEach(key => {
        const dataFetcher = route.resolve[key];
        if (reusableResolveData &&
          reusableResolveData[route.name] &&
          reusableResolveData[route.name][key]) {
          // Use parent's data.
          tasks.push({
            name: route.name,
            key,
          ...
          // Normalize the function return result with promise
          tasks.push(
            Promise.resolve(dataFetcher(params)).then(result => ({name: route.name, key, result}))
          );
        }
      });
    });
